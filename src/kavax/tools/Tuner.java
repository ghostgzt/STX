...
public class Tuner  implements CommandListener, PlayerListener {
    ...
    public Tuner() {
        ...
        textBox = new TextBox("Radio Tuner", "starting...", MAX_TEXT_SIZE, TextField.ANY|TextField.UNEDITABLE);
        ...
        display = kavax.microedition.lcdui.MIDtxt.dp;
    ...
    private void startAction() {
        display.callSerially(new Runnable() {
            public void run() {
                textBox.setString(Tuner.this.toSay + 
                        " from \n" + 
                        (int) (Tuner.this.freq / 10) + 
                        "kHz...");
                Tuner.this.toRun.start();
            }
        });
    }
    ...
    class SearchUpThread extends Thread {
        public void run() {
            Tuner.this.searchUp();
            Tuner.this.toRun = null;
        }
    }

    class SearchDownThread extends Thread {
        public void run() {
            Tuner.this.searchDown();
            Tuner.this.toRun = null;
        }
    }
