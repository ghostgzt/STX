...
public class ModelPPM {
    ...
    public boolean decodeInit(Unpack unpackRead, int escChar) throws IOException{
    ...
    public int decodeChar() throws IOException{
        ...
        if (minContext.getNumStats() != 1) {
            if (minContext.getFreqData().getStats() <= subAlloc.getPText() || minContext.getFreqData().getStats() > subAlloc.getHeapEnd()) {
                return (-1);
            }
        ...
        if ((orderFall == 0) && foundState.getSuccessor() > subAlloc.getPText()) {
            ...
            updateModel();
            //this.foundState.setAddress(foundState.getAddress());
            if (escCount == 0) {
                clearMask();
            }
        }
    ...
    private int /* ppmcontext ptr */ createSuccessors(boolean Skip,
        ...
        if (pc.getNumStats() != 1) {
            ...
            upState.setFreq(1 + ((2 * cf <= s0) ? (5 * cf > s0 ? 1 : 0) : ((2 * cf + 3 * s0 - 1) / (2 * s0))));
        } else {
...
//								.getSummFreq() <= 8 * ns1) ? 1 : 0));
    ...
    private void fill(int[] data, int j) {
