...
public class SubAllocator {
    ...
    public boolean startSubAllocator(int SASize) {
        ...
        stopSubAllocator();
        t = Math.min(t, MemorySet.RARPPMMEM);
        int allocSize = t / FIXED_UNIT_SIZE * UNIT_SIZE + UNIT_SIZE;
        //   System.out.println("allocSize=" + chen.c.showSize(allocSize));
    ...
    //    assert (realAllocSize - tempMemBlockPos == RarMemBlock.size) : realAllocSize + " " + tempMemBlockPos + " " + RarMemBlock.size;
    ...
    public void initSubAllocator() {
        ...
        k = freeListPos + sizeOfFreeList();
        for (i = freeListPos; i < k; i++) {
            heap[i] = 0;
        }
        pText = heapStart;
